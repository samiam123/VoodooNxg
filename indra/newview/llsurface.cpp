...
S32 LLSurface::sTextureSize = 1024;// humm let's see what happens if this is not a fixed size sams voodoo
...
void LLSurface::connectNeighbor(LLSurface *neighborp, U32 direction)
{
	...
	//ppe[0] = std::min(mPatchesPerEdge,neighborPatchesPerEdge); // used for x//dosent like to build with std
	//ppe[1] = std::min(mPatchesPerEdge,neighborPatchesPerEdge); // used for y//dosent same here too -VS
	
	ppe[0] = (mPatchesPerEdge < neighborPatchesPerEdge) ? mPatchesPerEdge : neighborPatchesPerEdge; // used for x //added -VS
	ppe[1] = ppe[0]; // used for y //added -VS
	...
	if(own_ypos >= neighbor_ypos) {
		...
		//ppe[1] = std::min(mPatchesPerEdge, neighborPatchesPerEdge-neighbor_offset[1]);//org
		ppe[1] = min(mPatchesPerEdge, neighborPatchesPerEdge-neighbor_offset[1]);
		
	}
	else {
		own_offset[1] = (neighbor_ypos - own_ypos) / mGridsPerPatchEdge;
		//ppe[1] = std::min(mPatchesPerEdge-own_offset[1], neighborPatchesPerEdge);//org
	...
	if(own_xpos >= neighbor_xpos) {
		...
		//ppe[0] = std::min(mPatchesPerEdge, neighborPatchesPerEdge-neighbor_offset[0]);//org
        ppe[0] = min(mPatchesPerEdge, neighborPatchesPerEdge-neighbor_offset[0]);
		
	}
	else {
		own_offset[0] = (neighbor_xpos - own_xpos) / mGridsPerPatchEdge;
		//ppe[0] = std::min(mPatchesPerEdge-own_offset[0], neighborPatchesPerEdge);//org
