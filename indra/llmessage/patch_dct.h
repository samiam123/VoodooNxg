...
//	U16	patchids;		// 2 = 10 (actually only uses 10 bits, 5 for each)//non var
//Buzz this was comment out now anyway in newer singu none of this seems used added line to public below
//} LLPatchHeader;
class LLPatchHeader
{
public:
	...
	U8	quant_wbits;	// 1 = 8 (upper 4 bits is quant - 2, lower 4 bits is word bits - 2)
	//U16	patchids;		// 2 = 10 (actually only uses 10 bits, 5 for each) // non var
	U32  patchids;      // 2 = 10 (actually only uses 10 bits, 5 for each)// with var
};
